...
export class ProgressIndicator {
  ...
  startSpinner(operation, customText = null, id = null) {
    ...
    const config =
      this.spinnerConfigs[this.theme]?.[operation] ||
      this.spinnerConfigs.default[operation];
  ...
  succeedSpinner(id, message = null) {
    const spinnerData = this.activeSpinners.get(id);
    if (spinnerData) {
      const duration = this.showTime
        ? this.formatDuration(Date.now() - spinnerData.startTime)
        : '';
      const successMessage =
        message ||
        `${spinnerData.text.replace('...', '')} completed${duration}`;
  ...
  warnSpinner(id, message = null) {
    ...
    if (spinnerData) {
      const warningMessage =
        message || `${spinnerData.operation} completed with warnings`;
    ...
    const progressBar = {
      ...
      render: () => {
        ...
        const filled = Math.round(
          (progressBar.current / progressBar.total) * 20
        );
        const bar = '█'.repeat(filled) + '░'.repeat(20 - filled);
        const duration = this.showTime
          ? this.formatDuration(Date.now() - progressBar.startTime)
          : '';

        console.log(
          `\r${progressBar.text} [${bar}] ${percentage}% (${progressBar.current}/${progressBar.total})${duration}`
        );
      },
  ...
  showMultiStepProgress(steps, title = 'Operation Progress') {
    ...
    const multiStep = {
      ...
      next: (customText = null) => {
        if (multiStep.currentStep < steps.length) {
          ...
          const spinner =
            multiStep.spinners[multiStep.currentStep] ||
      ...
      fail: (stepIndex, message = null) => {
        if (
          stepIndex < multiStep.spinners.length &&
          multiStep.spinners[stepIndex]
        ) {
    ...
    logger.debug('Created multi-step progress', {
      title,
