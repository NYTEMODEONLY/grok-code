  ...
  displayDiff(diffText, options = {}) {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const processedLine = this.processDiffLine(
          line,
          currentFile,
          currentHunk,
          inHunk,
  ...
  processDiffLine(line, currentFile, currentHunk, inHunk, options) {
    ...
    if (line.startsWith('+') && !line.startsWith('+++')) {
      ...
      if (currentFile && options.syntaxHighlight !== false) {
        const language = this.detectLanguageFromFile(
          currentFile.new || currentFile.old
        );
        if (language) {
    ...
    if (line.startsWith('-') && !line.startsWith('---')) {
      ...
      if (currentFile && options.syntaxHighlight !== false) {
        const language = this.detectLanguageFromFile(
          currentFile.old || currentFile.new
        );
        if (language) {
          const highlighted = this.syntaxHighlighter.highlight(
            content,
...
function goodbye(name) {
  ...
  getStats() {
    return {
      ...
      contextLines: this.contextLines,
      theme: this.theme,
      syntaxHighlighterEnabled: this.syntaxHighlighter
        ? !this.syntaxHighlighter.enabled
        : false,
    };
  }
}
