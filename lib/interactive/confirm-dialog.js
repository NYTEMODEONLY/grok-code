...
export class ConfirmDialog {
  ...
  showHeader(operation, type) {
    ...
    console.log(`${color}╔══════════════════════════════════════════════════════════════════════════════════════════════════╗${resetColor}`);
    console.log(`${color}║${resetColor} ${icon} ${operation || 'Confirmation Required'} ${' '.repeat(Math.max(0, 95 - (operation?.length || 20)))} ${color}║${resetColor}`);
    console.log(`${color}╚══════════════════════════════════════════════════════════════════════════════════════════════════╝${resetColor}`);
  }
  ...
  async showFilePreviews(files, previewOptions = {}) {
    ...
    for (const filePath of files) {
      try {
        ...
        if (stats.size > 0 && stats.size < 10000) { // Only preview small files
          const content = await fs.readFile(filePath, 'utf8');
          const lines = content.split('\n');

          // Show first few lines with syntax highlighting
          const previewLines = lines.slice(0, this.maxPreviewLines);
          const highlighted = this.syntaxHighlighter.highlight(content, extension);

          if (showLineNumbers) {
            highlighted.split('\n').slice(0, this.maxPreviewLines).forEach((line, index) => {
          ...
          if (lines.length > this.maxPreviewLines) {
            console.log(`... (${lines.length - this.maxPreviewLines} more lines)`);
          }
  ...
  async showChangesPreview(changes, previewOptions = {}) {
    ...
    for (const change of changes) {
      ...
      console.log(`📝 ${type.toUpperCase()}: ${path.relative(process.cwd(), file)}`);
      console.log('─'.repeat(50));
  ...
  showImpactAssessment(impact) {
    ...
    console.log(`🔴 Risk Level: ${color}${riskLevel.toUpperCase()}${resetColor}`);
  ...
  async confirmFileOperation(options = {}) {
    ...
    return this.confirm({
      message: description,
      type: impact.riskLevel === 'high' || impact.riskLevel === 'critical' ? 'danger' : 'warning',
  ...
  async confirmWithTyping(options = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      const { typedPhrase } = await inquirer.prompt([
        {
          ...
          validate: (input) => {
            ...
            const normalizedPhrase = caseSensitive ? confirmationPhrase : confirmationPhrase.toLowerCase();
      ...
      const normalizedInput = caseSensitive ? typedPhrase : typedPhrase.toLowerCase();
      const normalizedPhrase = caseSensitive ? confirmationPhrase : confirmationPhrase.toLowerCase();
  ...
  generateFileWarnings(files, action) {
    ...
    if (criticalFiles.length > 0) {
      warnings.push(`Critical files detected: ${criticalFiles.map(f => path.basename(f)).join(', ')}`);
    }
  ...
  assessFileImpact(files, action) {
    ...
    files.forEach(file => {
      ...
      if (['.json', '.js', '.ts', '.py'].includes(ext)) {
        riskLevel = 'medium';
      } else if (['package.json', 'config', '.env'].some(critical => file.includes(critical))) {
        riskLevel = 'high';
      }
    ...
    if (files.some(f => f.includes('requirements.txt') || f.includes('Pipfile'))) {
    ...
    return {
      ...
      dependencies,
      performance: action === 'delete' ? 'Potential performance improvement' : null
    };
  }
  ...
  getStatistics() {
    ...
    const confirmed = this.confirmationHistory.filter(c => c.confirmed).length;
    ...
    return {
      ...
      lastConfirmation: this.confirmationHistory[this.confirmationHistory.length - 1]
    };
  }
