...
export class CodeSearch {
  constructor(options = {}) {
    ...
    this.codePreview = options.codePreview || new CodePreview({ syntaxHighlighter: this.syntaxHighlighter });
    this.searchHistory = [];
  ...
  async search(query, options = {}) {
    ...
    logger.debug('Starting code search', { query, mode: opts.mode, directory: opts.directory });
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (stats.isDirectory()) {
            ...
            if (['node_modules', '.git', '.vscode', '.idea', 'dist', 'build'].includes(item)) {
              continue;
            }
            ...
            const fileResults = await this.searchInFile(fullPath, query, options);
        ...
        logger.debug('Error processing directory', { path: dirPath, error: error.message });
      }
    };
  ...
  async searchInFile(filePath, query, options) {
    try {
      ...
      logger.debug('Error searching file', { file: filePath, error: error.message });
      return [];
    }
  }
  ...
  createMatchPreview(lines, matchLine, query, options) {
    ...
    for (let i = startLine; i <= endLine; i++) {
      ...
      if (isMatchLine) {
        ...
        const highlightRegex = new RegExp(`(${escapedQuery})`, options.caseSensitive ? 'gi' : 'g');
        highlightedLine = line.replace(highlightRegex, '\x1b[43m\x1b[30m$1\x1b[0m'); // Yellow background, black text
      }

      const marker = isMatchLine ? '▶' : '│';
      previewLines.push(`${lineNumber.toString().padStart(4, ' ')} ${marker} ${highlightedLine}`);
    }
  ...
  async interactiveSearch(options = {}) {
    ...
    for (const [filePath, fileResults] of Object.entries(fileGroups)) {
      ...
      fileResults.forEach(result => {
        console.log(`  ${matchCount}. Line ${result.line}: ${this.truncateLine(result.content, 80)}`);
        matchCount++;
      });
      console.log();
    }
  ...
  async testSearch(testType = 'basic') {
    ...
    if (testType === 'basic' || testType === 'all') {
      ...
      console.log(`Found ${results.length} matches for "function" in JS files:`);
      results.forEach((result, index) => {
        console.log(`  ${index + 1}. ${result.relativePath}:${result.line} - ${this.truncateLine(result.content, 50)}`);
      });
    }
