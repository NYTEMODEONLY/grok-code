...
export class AutoContextBuilder {
  constructor() {
    ...
    this.autoAddTriggers = {
      keywords: [
        'fix', 'implement', 'add', 'create', 'update', 'modify', 'change',
        'refactor', 'optimize', 'improve', 'debug', 'test', 'build'
      ],
   ...
   */
  async analyzeAndAutoAdd(userInput, fileContext, conversationHistory = [], options = {}) {
    const {
    ...
    try {
      ...
      const suggestions = await fileSuggester.suggestFiles(userInput, ['.'], {
      ...
      const candidates = suggestions.suggestions.filter(suggestion => {
        ...
        const alreadyInContext = fileContext.hasOwnProperty(suggestion.file.path);
        return confidence >= confidenceThreshold && !alreadyInContext;
      });

      if (candidates.length === 0) {
        return { autoAdded: false, reason: 'insufficient confidence or files already in context' };
      }
      ...
      if (addedFiles.length > 0) {
        ...
        this.learnFromAddition(userInput, addedFiles, suggestions.task.analysis);
  ...
  shouldTriggerAutoAdd(userInput) {
    ...
    const codingIndicators = [
      ...
      'component', 'module', 'api', 'database', 'error', 'bug',
  ...
  getStatistics() {
    ...
    const autoAddedFiles = Array.from(this.contextHistory.values())
      .filter(entry => entry.autoAdded);

    const taskTypeStats = {};
    for (const pattern of this.queryPatterns.values()) {
      taskTypeStats[pattern.taskType] = (taskTypeStats[pattern.taskType] || 0) + pattern.frequency;
    }
  ...
  configure(config) {
    if (config.autoAddTriggers) {
      this.autoAddTriggers = { ...this.autoAddTriggers, ...config.autoAddTriggers };
    }
