...
export class RelevanceScorer {
  constructor() {
    ...
    this.stopWords = new Set([
      ...
      'them', 'my', 'your', 'his', 'its', 'our', 'their', 'what', 'when', 'where', 'why', 'how'
    ]);
  ...
  async scoreFileRelevance(query, filePaths, options = {}) {
    ...
    for (const filePath of files) {
      try {
        const score = await this.scoreSingleFile(filePath, queryTerms, dependencyGraph, { cacheEnabled });
   ...
   */
  async scoreSingleFile(filePath, queryTerms, dependencyGraph = null, options = {}) {
    ...
    try {
      ...
      if (dependencyGraph) {
        const depScore = this.scoreDependencyRelevance(filePath, queryTerms, dependencyGraph);
