...
export class FileSuggester {
  constructor() {
    ...
    this.taskPatterns = {
      bugfix: [
        /\bfix\b/i, /\bbug\b/i, /\berror\b/i, /\bissue\b/i, /\bproblem\b/i,
        /\bcrash\b/i, /\bfailing\b/i, /\bbroken\b/i, /\bdebug\b/i
      ],
      feature: [
        /\badd\b/i, /\bimplement\b/i, /\bcreate\b/i, /\bnew\b/i, /\bfeature\b/i,
        /\bbuild\b/i, /\bdevelop\b/i, /\bmake\b/i
      ],
      refactor: [
        /\brefactor\b/i, /\brestructure\b/i, /\bclean\b/i, /\boptimize\b/i,
        /\bsimplify\b/i, /\bimprove\b/i, /\bmodular/i, /\breorganize\b/i
      ],
      test: [
        /\btest\b/i, /\bspec\b/i, /\bunit\b/i, /\bintegration\b/i,
        /\bcoverage\b/i, /\bmock\b/i, /\bstub\b/i
      ],
      config: [
        /\bconfig\b/i, /\bsetup\b/i, /\binstall\b/i, /\bdeploy\b/i,
        /\bbuild\b/i, /\bwebpack\b/i, /\bbabel\b/i, /\bpackage\b/i
      ],
      documentation: [
        /\bdocument\b/i, /\breadme\b/i, /\bcomment\b/i, /\bdoc\b/i,
        /\bguide\b/i, /\btutorial\b/i, /\bexample\b/i
      ],
      performance: [
        /\bperformance\b/i, /\bspeed\b/i, /\bfast\b/i, /\bslow\b/i,
        /\boptimize\b/i, /\bcache\b/i, /\bmemory\b/i
      ],
      security: [
        /\bsecurity\b/i, /\bauth\b/i, /\bencrypt\b/i, /\bsecure\b/i,
        /\bvulnerab/i, /\battack\b/i, /\bhack\b/i
      ]
    };
  ...
  async suggestFiles(task, filePaths, options = {}) {
    ...
    console.log(`Task analysis: ${taskAnalysis.type} - ${taskAnalysis.confidence}% confidence`);

    // Get relevance scores for files
    const relevanceResults = await relevanceScorer.scoreFileRelevance(task, filePaths, {
    ...
    const weightedResults = this.applyTaskWeighting(relevanceResults, taskAnalysis);
    ...
    const suggestions = this.generateSuggestions(weightedResults, taskAnalysis, contextOptimized, {
      maxSuggestions,
      detailLevel,
    });
  ...
  analyzeTask(task, forcedType = null) {
    ...
    const totalMatches = Object.values(matches).reduce((sum, m) => sum + m.count, 0);
    const confidence = totalMatches > 0 ? Math.min(100, (bestScore / totalMatches) * 100) : 30;
  ...
  getTaskCharacteristics(taskType) {
    const characteristics = {
      bugfix: {
        ...
        riskLevel: 'medium',
        typicalFiles: ['*.test.js', '*.spec.js', 'error-handler.js', 'logger.js'],
  ...
  applyTaskWeighting(relevanceResults, taskAnalysis) {
    ...
    return relevanceResults.map(result => {
      let weight = 1.0;
      ...
      const confidenceMultiplier = 0.8 + (taskAnalysis.confidence / 100) * 0.4; // 0.8 to 1.2
  ...
  categorizeFile(filePath) {
    ...
    if (fileName.includes('config') || fileName.includes('.config.') ||
        fileName.includes('package.json') || fileName.includes('webpack') ||
        fileName.includes('babel') || fileName.includes('eslint')) {
      return 'config';
    }

    if (fileName.includes('readme') || fileName.endsWith('.md') ||
        fileName.includes('doc') || fileName.includes('guide')) {
      return 'docs';
    }
   ...
   */
  generateSuggestions(weightedResults, taskAnalysis, contextOptimized, options) {
    const { maxSuggestions, detailLevel } = options;
    const suggestions = [];

    // Use context-optimized results if available, otherwise use weighted results
    const candidates = contextOptimized ? contextOptimized.files.map(f => ({
      filePath: f.filePath,
  ...
  generateRecommendations(suggestions, taskAnalysis) {
    ...
    if (suggestions.length === 0) {
      return ['No specific file recommendations available. Consider exploring the codebase manually.'];
    }

    // Primary recommendation
    const topSuggestion = suggestions[0];
    recommendations.push(`🔍 Start with ${topSuggestion.file.name} - ${topSuggestion.reasoning.toLowerCase()}`);

    // Task-specific recommendations
    const taskRecs = this.getTaskRecommendations(taskAnalysis.type, suggestions);
    recommendations.push(...taskRecs);

    // General workflow recommendations
    if (suggestions.length > 3) {
      recommendations.push(`📋 Review top ${Math.min(3, suggestions.length)} files first, then expand as needed`);
    }

    // Risk-based recommendations
    const characteristics = taskAnalysis.characteristics;
    if (characteristics.riskLevel === 'high' || characteristics.riskLevel === 'critical') {
      recommendations.push(`⚠️ High-risk task: Consider creating backups and thorough testing`);
    }
  ...
  getTaskRecommendations(taskType, suggestions) {
    ...
    switch (taskType) {
      case 'bugfix':
        recommendations.push('🐛 Focus on understanding the bug before making changes');
        recommendations.push('🧪 Run existing tests to establish baseline behavior');
        break;

      case 'feature':
        recommendations.push('✨ Plan the feature implementation and required changes');
        recommendations.push('📝 Consider updating documentation and examples');
        break;

      case 'refactor':
        recommendations.push('🔄 Ensure comprehensive test coverage before refactoring');
        recommendations.push('📊 Consider performance implications of changes');
        break;

      case 'test':
        recommendations.push('✅ Follow testing best practices and naming conventions');
        recommendations.push('🎯 Aim for meaningful test coverage and edge cases');
        break;

      case 'security':
        recommendations.push('🔒 Consult security best practices and guidelines');
        recommendations.push('⚡ Consider security implications and attack vectors');
        break;

      case 'performance':
        recommendations.push('⚡ Profile current performance before optimization');
        recommendations.push('📈 Measure improvements and ensure no regressions');
        break;
    }
  ...
  configure(config) {
    ...
    if (config.taskFileWeights) {
      this.taskFileWeights = { ...this.taskFileWeights, ...config.taskFileWeights };
    }
  }
}
