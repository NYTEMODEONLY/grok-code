...
export class ContextWindowOptimizer {
  ...
  async optimizeContext(query, filePaths, options = {}) {
    ...
    const tokenLimit = customTokenLimit || this.modelLimits[model] || this.modelLimits.default;

    // Get relevance scores for all files
    const relevanceResults = await relevanceScorer.scoreFileRelevance(query, filePaths, {
    ...
    for (const file of selectedFiles) {
      try {
        const optimizedContent = await this.optimizeFileContent(file.filePath, query, {
          includeSymbols,
          maxSize: this.calculateFileSizeLimit(selectedFiles.length, tokenLimit),
        });
        ...
        console.warn(`Failed to optimize content for ${file.filePath}: ${error.message}`);
      }
    }
  ...
  selectDiverseFiles(relevanceResults, maxFiles, query) {
    ...
    for (const file of remainingFiles) {
      ...
      if (score.symbolMatches > score.keywordMatches && score.symbolMatches > score.pathScore) {
  ...
  async optimizeFileContent(filePath, query, options = {}) {
    ...
    try {
      ...
      const sections = this.extractFileSections(content, lines, filePath, query);
      ...
      return {
        ...
        sections: sections.map(s => ({ type: s.type, lines: s.lines.length })),
        optimization: {
          originalSize: content.length,
          optimizedSize: optimizedContent.length,
          compressionRatio: Math.round((optimizedContent.length / content.length) * 100),
  ...
  extractFileSections(content, lines, filePath, query) {
    ...
    if (path.extname(filePath).match(/\.(js|ts|py)$/)) {
      ...
      if (signatures.lines.length > 0) {
        sections.push({
          ...
          relevance: this.calculateSectionRelevance(signatures.content, queryTerms),
        });
      }
    }
    ...
    const implementationLines = lines.filter((line, index) =>
    ...
    if (implementationLines.length > 0) {
      ...
      sections.push({
        ...
        relevance: this.calculateSectionRelevance(implementationContent, queryTerms),
      });
    }
    ...
    for (const section of sections) {
      ...
      if (currentSize + sectionSize <= maxSize) {
        ...
        if (remainingSpace > 50) { // Only add if we have meaningful space
          const truncated = this.truncateContent(sectionContent, remainingSpace);
          content += truncated + '\n\n';
  ...
  generateSummary(contextFiles, query) {
    ...
    return `Optimized context for "${query}": ${contextFiles.length} files, ` +
