...
export class ConventionAutoApplier {
  ...
  async applyToFile(filePath, options = {}) {
    try {
      ...
      if (this.teamPatternsLearner) {
        teamPrefs.naming = this.teamPatternsLearner.getTeamPreferences('naming');
        teamPrefs.style = this.teamPatternsLearner.getTeamPreferences('style');
        teamPrefs.structure = this.teamPatternsLearner.getTeamPreferences('structure');
      }

      // Apply fixes
      const result = await this.applyFixes(content, language, conventions, teamPrefs);
      ...
      logger.error('Failed to apply conventions to file', { filePath, error: error.message });
      return {
        filePath,
        error: error.message,
        modified: false,
  ...
  async applyToFiles(filePaths, options = {}) {
    ...
    for (const filePath of filePaths) {
      const result = await this.applyToFile(filePath, { ...options, conventions });
      ...
      if (result.suggestions) {
        results.suggestions.push(...result.suggestions.map(s => ({ file: filePath, ...s })));
      }
    }
  ...
  async applyFixes(content, language, conventions = {}, teamPrefs = {}) {
    ...
    try {
      // Quote style fixes
      const quoteResult = this.fixQuoteStyle(newContent, conventions, teamPrefs);
      ...
      // Indentation fixes
      const indentResult = this.fixIndentation(newContent, conventions, teamPrefs);
      ...
      // Semicolon fixes
      const semicolonResult = this.fixSemicolons(newContent, conventions, teamPrefs);
      ...
      const commaResult = this.fixTrailingCommas(newContent, language, conventions, teamPrefs);
      ...
      const namingResult = this.fixNamingConventions(newContent, language, conventions, teamPrefs);
  ...
  fixQuoteStyle(content, conventions, teamPrefs) {
    ...
    if (teamPrefs.style?.preferences) {
      const quotePrefs = Object.keys(teamPrefs.style.preferences).filter(p => p.includes('quote'));
  ...
  fixIndentation(content, conventions, teamPrefs) {
    ...
    if (teamPrefs.style?.preferences) {
      const indentPrefs = Object.keys(teamPrefs.style.preferences).filter(p => p.includes('indent'));
  ...
  fixSemicolons(content, conventions, teamPrefs) {
    ...
    // Check team preferences
    if (teamPrefs.style?.preferences) {
      const semicolonPrefs = Object.keys(teamPrefs.style.preferences).filter(p => p.includes('semicolon'));
  ...
  async createBackup(filePath, content) {
    try {
      ...
      logger.debug('Created backup file', { original: filePath, backup: backupPath });
    } catch (error) {
      logger.warn('Failed to create backup', { filePath, error: error.message });
    }
  }
