...
export class TeamPatternsLearner {
  constructor(options = {}) {
    ...
    this.patternsFile = path.join(this.projectRoot, '.grok', 'team-patterns.json');
    ...
    this.loadPatterns();
    logger.info('Team patterns learner initialized', {
  ...
  async loadPatterns() {
    try {
      ...
      logger.warn('Failed to load team patterns, starting fresh', { error: error.message });
    }
  }
  ...
  updatePatterns(interaction) {
    ...
    if (type === 'accepted') {
      categoryPatterns.accepted[pattern] = (categoryPatterns.accepted[pattern] || 0) + 1;
    } else if (type === 'rejected') {
      categoryPatterns.rejected[pattern] = (categoryPatterns.rejected[pattern] || 0) + 1;
    } else if (type === 'corrected' && correction) {
      // Learn that user prefers correction over pattern
      categoryPatterns.rejected[pattern] = (categoryPatterns.rejected[pattern] || 0) + 1;
      categoryPatterns.accepted[correction] = (categoryPatterns.accepted[correction] || 0) + 1;
    }
  ...
  updateConfidence(category) {
    ...
    const totalAccepted = Object.values(catPatterns.accepted).reduce((a, b) => a + b, 0);
    const totalRejected = Object.values(catPatterns.rejected).reduce((a, b) => a + b, 0);
    ...
    const allPatterns = new Set([...Object.keys(catPatterns.accepted), ...Object.keys(catPatterns.rejected)]);
    ...
    for (const pattern of allPatterns) {
      ...
      if (total > 0) {
        ...
        const confidence = (p + z*z/(2*n) - z * Math.sqrt((p*(1-p) + z*z/(4*n))/n)) / (1 + z*z/n);
  ...
  getTeamPreferences(category, context = {}) {
    ...
    for (const [pattern, confidence] of Object.entries(catPatterns.confidence)) {
   ...
   * @param {Object} context - Additional context
   */
  learnFromCorrection(originalPattern, correctedPattern, category, context = {}) {
  ...
  async endSession() {
    ...
    this.currentSession.summary = {
      ...
      categories: [...new Set(this.currentSession.interactions.map(i => i.category))],
  ...
  getStatistics() {
    ...
    for (const category of ['naming', 'style', 'structure']) {
      if (this.patterns[category]) {
        ...
        stats.categories[category] = {
          ...
          confident: Object.values(cat.confidence).filter(c => c >= this.confidenceThreshold).length
        };

        // Average confidence
        const confidences = Object.values(cat.confidence);
        if (confidences.length > 0) {
          stats.avgConfidence[category] = confidences.reduce((a, b) => a + b, 0) / confidences.length;
        }
      }
    }
    ...
    for (const category of ['naming', 'style', 'structure']) {
      if (this.patterns[category]) {
        Object.keys(this.patterns[category].accepted).forEach(p => allPatterns.add(p));
        Object.keys(this.patterns[category].rejected).forEach(p => allPatterns.add(p));
      }
    }
