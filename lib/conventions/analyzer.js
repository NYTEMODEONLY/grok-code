...
export class ConventionAnalyzer {
  ...
  async analyzeProject() {
    try {
      ...
      logger.error('Failed to analyze project conventions', {
        error: error.message,
      });
      throw error;
    }
  ...
  async analyzeFile(filePath) {
    try {
      ...
      logger.debug('Failed to analyze file', {
        filePath,
        error: error.message,
      });
    }
  ...
  analyzeCodeStyle(content, lines, language) {
    ...
    const statements =
      content.match(/[a-zA-Z_$][a-zA-Z0-9_$]*\s*[=+\-*/]\s*[^;]*$/gm) || [];
  ...
  analyzeNaming(content, language) {
    ...
    if (!naming.functions)
      naming.functions = { camelCase: 0, PascalCase: 0, snake_case: 0 };
  ...
  analyzeImports(content, language) {
    ...
    if (language === 'javascript' || language === 'typescript') {
      ...
      const defaultImports = (
        content.match(/import\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s+from/g) || []
      ).length;
      const namespaceImports = (
        content.match(/import\s*\*\s*as\s+[a-zA-Z_$]/g) || []
      ).length;
      ...
      if (!imports.organization)
        imports.organization = { grouped: 0, alphabetical: 0 };
  ...
  analyzeStructure(content, lines, language) {
    ...
    if (language === 'javascript' || language === 'typescript') {
      const namedExports = (
        content.match(/export\s+(?:const|let|var|function|class)/g) || []
      ).length;
  ...
  analyzeComments(content, lines, language) {
    ...
    if (!comments.style)
      comments.style = { singleLine: 0, multiLine: 0, jsdoc: 0 };
    ...
    // Comment density
    if (!comments.density)
      comments.density = { totalLines: 0, commentLines: 0 };
  ...
  analyzePatterns(content, language) {
    ...
    if (!patterns.errorHandling)
      patterns.errorHandling = { tryCatch: 0, promises: 0, asyncAwait: 0 };
    ...
    if (language === 'javascript' || language === 'typescript') {
      ...
      const regularFunctions = (content.match(/function\s+\w+\s*\(/g) || [])
        .length;
  ...
  analyzeTesting(content, filePath, language) {
    ...
    if (!testing.frameworks)
      testing.frameworks = { jest: 0, mocha: 0, jasmine: 0, pytest: 0 };
  ...
  calculatePreferences() {
    ...
    if (semicolons) {
      prefs.semicolons =
        semicolons.used > semicolons.omitted ? 'required' : 'optional';
    }

    // Naming preferences
    const naming = this.conventions.naming;
    if (naming && naming.variables) {
      const varStyle = Object.entries(naming.variables).sort(
        ([, a], [, b]) => b - a
      )[0][0];
  ...
  calculateStatistics() {
    ...
    if (lengths.length > 0) {
      stats.avgLineLength = Math.round(
    ...
    if (density && density.totalLines > 0) {
      stats.commentDensity = Math.round(
        (density.commentLines / density.totalLines) * 100
      );
    }

    // Average function length
    const funcLengths =
      this.conventions.structure.functionLength?.lengths || [];
    if (funcLengths.length > 0) {
      stats.avgFunctionLength = Math.round(
