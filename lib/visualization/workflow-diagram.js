...
export class WorkflowDiagram {
  ...
  generateDiagram(plan, options = {}) {
    try {
      ...
      logger.error('Failed to generate workflow diagram', { error: error.message });
      return this.generateErrorDiagram(`Diagram generation failed: ${error.message}`);
    }
  }
  ...
  generateFlowchart(plan, compact = false) {
    ...
    features.forEach(feature => {
      ...
      featureFiles.forEach(filePath => {
        fileNodes.push({
          id: filePath,
          label: this.truncateText(this.getFileName(filePath), compact ? 12 : 20),
          type: 'file',
          feature: feature,
    ...
    flows.forEach(([from, to]) => {
      ...
      if (fromNode && toNode) {
        this.drawConnection(canvas, fromNode.x, fromNode.y, toNode.x, toNode.y, 'flow');
      }
    });

    // Draw feature-to-file connections
    featureNodes.forEach(featureNode => {
      const featureFiles = fileNodes.filter(n => n.feature === featureNode.id);
      featureFiles.forEach(fileNode => {
        this.drawConnection(canvas, featureNode.x, featureNode.y, fileNode.x, fileNode.y, 'dependency');
      });
    });
  ...
  generateMindMap(plan, compact = false) {
    ...
    features.forEach((feature, index) => {
      const branchSymbol = index === features.length - 1 ? this.currentSymbols.corner : this.currentSymbols.branch;
      ...
      featureFiles.forEach((fileName, fileIndex) => {
        const fileSymbol = fileIndex === featureFiles.length - 1 ? '    └──' : '    ├──';
  ...
  generateDependencyGraph(plan, compact = false) {
    ...
    Object.entries(depMap).forEach(([source, targets], index) => {
      ...
      targets.forEach((target, targetIndex) => {
        ...
        const prefix = isLast ? '    ' : `    ${this.currentSymbols.vertical}   `;
        const connector = targetIsLast ? this.currentSymbols.corner : this.currentSymbols.branch;
  ...
  generateOverview(plan) {
    ...
    features.forEach((feature, index) => {
      const featureFiles = Object.values(files).filter(f => f === feature).length;
...
`;
  ...
  drawNode(canvas, x, y, label, type) {
    ...
    const icon = type === 'feature' ? this.currentSymbols.feature : this.currentSymbols.file;
  ...
  drawConnection(canvas, x1, y1, x2, y2, type) {
    const symbol = type === 'flow' ? this.currentSymbols.flow : this.currentSymbols.arrow;
    ...
    if (connY < canvas.length) {
      for (let x = startX; x <= endX && x < canvas[connY].length; x++) {
        canvas[connY][x] = this.currentSymbols.connection[Math.min(x - startX, 2)];
      }
