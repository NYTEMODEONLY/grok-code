...
8. **WebFetch** - Fetch and analyze web content
   ...
   - Use for: reading documentation, checking APIs, fetching external resources

9. **WebSearch** - Search the web for information
   - Parameters: query (required), allowed_domains (optional), blocked_domains (optional)
   - Use for: finding solutions, researching technologies, discovering resources

10. **AskUserQuestion** - Ask the user questions interactively
    - Parameters: questions (required - array of question objects)
    - Use for: gathering preferences, clarifying requirements, getting decisions
...
9. **Parallel execution**: When multiple independent tool calls are needed, execute them in parallel for efficiency.

10. **Web resources**: Use WebFetch and WebSearch when you need external information not in the codebase.
...
`;
    ...
    while (iterations < this.maxToolIterations) {
      ...
      try {
        ...
        if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
          ...
          // Check if we can execute tools in parallel (independent tools)
          const canParallelize = this.canParallelizeCalls(assistantMessage.tool_calls);

          if (canParallelize && assistantMessage.tool_calls.length > 1) {
            // Execute tools in parallel
            this.emit('parallelExecution', { count: assistantMessage.tool_calls.length });
            ...
            // Add all results to messages and toolResults
   ...
   * @returns {boolean} Whether calls can be parallelized
   */
  canParallelizeCalls(toolCalls) {
    ...
    // Check for file conflicts - can't parallelize if multiple tools touch the same file
        ...
        try {
          ...
          if (filePath) {
            if (filePaths.has(filePath)) {
              // Same file touched by multiple tools
              return false;
            }
            filePaths.add(filePath);
          }
        } catch {
          // If we can't parse args, don't parallelize
          return false;
        }
      }
    }

    // Check for Bash commands that might conflict
    const bashCalls = toolCalls.filter(c => c.function.name === 'Bash');
    if (bashCalls.length > 1) {
      // Generally don't parallelize bash commands unless they're clearly independent
      return false;
    }
