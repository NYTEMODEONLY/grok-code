...
export class FilePlacementAdvisor {
  ...
  async suggestPlacement(fileName, fileType, context = {}) {
    try {
      logger.info('Analyzing file placement suggestion', {
        fileName,
        fileType,
      });
      ...
      const fileCharacteristics = this.analyzeFileCharacteristics(
        fileName,
        fileType,
        context
      );
      ...
      logger.error('Failed to suggest file placement', {
        error: error.message,
        fileName,
      });
      throw error;
  ...
  async analyzeProjectStructure() {
    ...
    try {
      ...
      analysis.filePatterns = this.analyzeFilePatterns(
        analysis.directoryStructure
      );
      analysis.layerMappings = this.mapLayersToDirectories(
        analysis.directoryStructure
      );
    } catch (error) {
      logger.warn('Project structure analysis partially failed', {
        error: error.message,
      });
    }
  ...
  analyzeFileCharacteristics(fileName, fileType, context) {
    ...
    characteristics.architecturalLayer = this.determineArchitecturalLayer(
      fileName,
      fileType
    );
    ...
    if (this.frameworkDetector) {
      characteristics.frameworkSpecific =
        this.determineFrameworkSpecificPlacement(fileName, fileType);
    }

    // Analyze file relationships
    characteristics.secondaryTypes = this.determineSecondaryTypes(
      fileName,
      fileType
    );
  ...
  async generatePlacementSuggestions(
    ...
    if (characteristics.frameworkSpecific) {
      suggestions.push(
        ...this.generateFrameworkSuggestions(characteristics, projectAnalysis)
      );
    }

    // Architecture-based suggestions
    suggestions.push(
      ...this.generateArchitectureSuggestions(characteristics, projectAnalysis)
    );

    // Convention-based suggestions
    if (projectAnalysis.conventions) {
      suggestions.push(
        ...this.generateConventionSuggestions(characteristics, projectAnalysis)
      );
    }

    // Existing pattern-based suggestions
    suggestions.push(
      ...this.generatePatternSuggestions(characteristics, projectAnalysis)
    );

    // Relationship-based suggestions
    if (characteristics.relatedFiles.length > 0) {
      suggestions.push(
        ...this.generateRelationshipSuggestions(
          characteristics,
          projectAnalysis
        )
      );
  ...
  analyzeFilePatterns(directoryStructure) {
    ...
    for (const [dirPath, info] of Object.entries(directoryStructure)) {
      if (info.purpose === 'components')
        patterns.componentDirectories.push(dirPath);
      if (info.purpose === 'services')
        patterns.serviceDirectories.push(dirPath);
  ...
  determineArchitecturalLayer(fileName, fileType) {
    ...
    for (const [layer, patterns] of Object.entries(
      this.placementRules.layers
    )) {
    ...
    for (const [type, regex] of Object.entries(
      this.placementRules.namingPatterns
    )) {
  ...
  analyzeNamingConvention(fileName) {
    ...
    for (const [convention, regex] of Object.entries(
      this.placementRules.namingPatterns
    )) {
  ...
  determineDirectoryLayer(dirPath, info) {
    ...
    if (
      ['controllers', 'views', 'templates', 'components', 'pages'].includes(
        dirName
      )
    ) {
      return 'presentation';
  ...
  validatePlacement(filePath, projectAnalysis) {
    ...
    if (!this.followsNamingConvention(fileName, projectAnalysis)) {
      validation.warnings.push(
        'File name may not follow project naming conventions'
      );
    }
