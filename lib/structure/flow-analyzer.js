...
export class FlowAnalyzer {
  ...
  async findServerEntryPoints() {
    ...
    try {
      ...
      const hasServerDep = serverDeps.some(
  ...
  async analyzeErrorFlows() {
    ...
    const errorFiles = await this.findFilesByPattern([
      '*error*.js',
      '*exception*.js',
    ]);
    ...
    const scanDirectory = async (dir, depth = 0) => {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (
            entry.isDirectory() &&
            !['.git', 'dist', 'build'].includes(entry.name)
          ) {
  ...
  containsServerCode(file) {
    try {
      ...
      return /(app\.listen|server\.listen|createServer|fastify|express\(\))/.test(
        content
      );
    } catch (error) {
      return false;
  ...
  extractCliCommands(file) {
    try {
      ...
      if (commandMatches) {
        commands.push(
          ...commandMatches.map((match) => match.replace(/\.command\(['"]/, ''))
        );
      }
  ...
  generateSummary() {
    return {
      entryPoints: this.flows.entryPoints.length,
      userJourneys: this.flows.userJourneys.length,
      apiEndpoints: this.flows.apiEndpoints.reduce(
