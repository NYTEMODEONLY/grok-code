...
export class ArchitectureMapper {
  constructor(options = {}) {
    ...
    this.architecturePatterns = {
      ...
      layered: {
        ...
        indicators: {
          folders: ['presentation', 'business', 'data', 'infrastructure', 'domain'],
      ...
      microservices: {
        ...
        layers: ['API Gateway', 'Services', 'Service Discovery', 'Infrastructure']
      },
      ...
      eventDriven: {
        ...
        layers: ['Event Publishers', 'Event Handlers', 'Event Store', 'Subscribers']
      }
    };
    ...
    const scanDirectory = async (dir, depth = 0) => {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (!this.includeNodeModules && relativePath.includes('node_modules')) {
            continue;
          }
          ...
          if (entry.isDirectory()) {
            ...
            } else if (['Dockerfile', 'docker-compose.yml'].includes(entry.name)) {
              structure.dockerFiles.push(relativePath);
            } else if (['webpack.config.js', 'rollup.config.js', 'vite.config.js'].includes(entry.name)) {
  ...
  async detectPatterns(structure) {
    ...
    for (const [patternKey, pattern] of Object.entries(this.architecturePatterns)) {
      let score = 0;
      let maxScore = 0;
    ...
    for (const [patternKey, data] of Object.entries(patternScores)) {
      if (data.confidence >= 60) { // 60% confidence threshold
    ...
    // Sort by confidence
    this.architecture.detectedPatterns.sort((a, b) => b.confidence - a.confidence);
  }
  ...
  classifyFileIntoLayer(file, pattern) {
    ...
    if (pattern.name === 'Model-View-Controller') {
      if (filePath.includes('/controllers/') || fileName.includes('controller')) {
        return 'Controller';
      }
    ...
    if (pattern.name === 'Layered Architecture') {
      if (filePath.includes('/presentation/') || fileName.includes('controller')) {
        return 'Presentation';
      }
    ...
    if (pattern.name === 'Clean Architecture') {
      ...
      if (filePath.includes('/interface-adapters/') || fileName.includes('presenter')) {
  ...
  classifyFileGeneric(file) {
    ...
    if (filePath.includes('/routes/') || filePath.includes('/controllers/') ||
        fileName.includes('router') || fileName.includes('controller')) {
      return 'Presentation';
    }

    // Business layer
    if (filePath.includes('/services/') || fileName.includes('service') ||
        fileName.includes('business') || fileName.includes('logic')) {
      return 'Business';
    }

    // Data layer
    if (filePath.includes('/models/') || filePath.includes('/repositories/') ||
        fileName.includes('model') || fileName.includes('repository') ||
  ...
  async identifyEntryPoints(structure) {
    ...
    const commonEntries = ['index.js', 'app.js', 'server.js', 'main.js', 'index.ts', 'app.ts'];
    ...
    if (structure.packageJson && structure.packageJson.scripts) {
      for (const [scriptName, scriptCommand] of Object.entries(structure.packageJson.scripts)) {
  ...
  async analyzeRelationships(structure) {
    ...
    for (const [layerName, files] of Object.entries(this.architecture.layers)) {
      relationships.layerDependencies[layerName] = this.analyzeLayerDependencies(layerName, files);
    }
  ...
  generateRecommendations() {
    ...
    if (this.architecture.detectedPatterns.length === 0) {
      recommendations.push({
        type: 'architecture',
        priority: 'medium',
        message: 'No clear architectural pattern detected. Consider adopting MVC, Layered, or Clean Architecture.',
        suggestion: 'Review project structure and consider implementing a clear architectural pattern.'
      });
    }
    ...
    if (layerCount < 2) {
      recommendations.push({
        ...
        suggestion: 'Consider separating concerns into distinct layers (Presentation, Business, Data).'
      });
    }
    ...
    if (layerCount > 5) {
      recommendations.push({
        ...
        suggestion: 'Review if all layers are necessary or if some can be consolidated.'
      });
    }
    ...
    if (this.architecture.entryPoints.length === 0) {
      recommendations.push({
        ...
        suggestion: 'Define clear application entry points in package.json or with standard naming.'
      });
    }
  ...
  generateSummary() {
    ...
    return {
      ...
      components: Object.values(this.architecture.layers).reduce((sum, files) => sum + files.length, 0),
