...
export class SafeFixApplier {
  ...
  async applyFix(fix, context) {
    ...
    try {
      ...
      if (applyResult.success) {
        // Validate the fix
        const validationResult = await this.validateFix(fix, context, applyResult);
        ...
        logger.error('Fix application failed, rolling back', { fixId, error: applyResult.error });
        await this.rollbackFix(fixId, context);
      ...
      logger.error('Safe fix application error', { fixId, error: error.message });
      // Attempt rollback on any error
      try {
        await this.rollbackFix(fixId, context);
      } catch (rollbackError) {
        logger.error('Rollback also failed', { fixId, rollbackError: rollbackError.message });
      }
  ...
  async runPreflightChecks(fix, context) {
    ...
    if (fix.changes && fix.changes.some(c => c.type === 'replace' || c.type === 'insert')) {
  ...
  requiresUserConfirmation(fix, context) {
    ...
    if ((fix.changes || []).length > this.confirmationThresholds.complexChanges) {
      return true;
    }

    // Critical files
    const affectedFiles = (fix.changes || []).map(c => c.file);
    if (affectedFiles.some(file =>
  ...
  async createBackups(fix, context, fixId) {
    ...
    for (const change of fix.changes || []) {
      ...
      if (fs.existsSync(filePath)) {
        const backupPath = path.join(this.backupDir, `${backupId}_${path.basename(change.file)}`);
   ...
   */
  getStats() {
    const successRate = this.fixStats.totalApplied > 0
