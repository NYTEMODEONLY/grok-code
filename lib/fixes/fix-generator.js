...
export class FixGenerator {
  ...
  async generateAndApplyFix(error, context, options = {}) {
    ...
    try {
      ...
      if (
        ...
        templateResult.confidence >= this.autoApplyThreshold
      ) {
        ...
        const applyResult = await this.safeApplier.applyFix(
          templateResult,
          context
        );
        this.recordFixResult(
          'template',
          applyResult,
          templateResult,
          startTime
        ...
        if (this.enableLearning && this.patterns) {
          this.patterns.recordFixAttempt(
            error,
            templateResult,
            applyResult.success,
            {
              ...
              rollbackReason: applyResult.rolledBack
                ? 'application_failed'
                : null,
            }
          );
        ...
        if (this.enablePersonalization && this.personalized && context.userId) {
          this.personalized.recordUserFeedback(
            context.userId,
            templateResult,
            applyResult.success,
            context
          );
          this.personalized.learnFromOutcome(
            context.userId,
            error,
            templateResult,
            applyResult.success,
      ...
      if (
        !templateResult.success ||
        templateResult.confidence < this.aiFallbackThreshold
      ) {
        ...
        if (
          aiResult.success &&
          aiResult.confidence >= this.autoApplyThreshold
        ) {
        ...
        if (templateResult.success && aiResult.success) {
          const hybridResult = await this.tryHybridFix(
            templateResult,
            aiResult,
            error,
            context
          ...
          if (
            ...
            hybridResult.confidence >= this.autoApplyThreshold
          ) {
            const applyResult = await this.safeApplier.applyFix(
              hybridResult,
              context
      ...
      const baseSuggestions = this.generateSuggestions(
        error,
        templateResult,
        context
      );
      ...
      if (this.enablePersonalization && this.personalized) {
        ...
        const personalizedRecs =
          this.personalized.getPersonalizedRecommendations(
            error,
            context,
            availableFixes
          );
      ...
      const preventionTips = this.enableLearning
        ? this.prevention.generatePreventionTips(context)
        : null;
  ...
  async tryHybridFix(templateFix, aiFix, error, context) {
    try {
      ...
      const betterFix =
        templateFix.confidence >= aiFix.confidence ? templateFix : aiFix;
  ...
  recordFixResult(method, applyResult, fix, startTime) {
    ...
    this.stats.averageConfidence =
      (currentAvg * (totalFixes - 1) + fix.confidence) / totalFixes;
