...
export class FixGenerator {
  constructor(options = {}) {
    ...
    this.personalized = options.personalized || new PersonalizedFixes({ patterns: this.patterns });
    this.prevention = options.prevention || new PreventionTips({
  ...
  async generateAndApplyFix(error, context, options = {}) {
    ...
    if (this.enableLearning && this.patterns) {
      this.patterns.recordFixAttempt(error, templateResult, applyResult.success, {
        duration: Date.now() - startTime,
        wasAutoApplied: true,
        sessionId: context.sessionId,
        userId: context.userId,
        projectId: context.projectId,
        rollbackReason: applyResult.rolledBack ? 'application_failed' : null
      });
    }

    // Record user feedback for personalization
    if (this.enablePersonalization && this.personalized && context.userId) {
      this.personalized.recordUserFeedback(context.userId, templateResult, applyResult.success, context);
      this.personalized.learnFromOutcome(context.userId, error, templateResult, applyResult.success, {
        ...context,
        duration: Date.now() - startTime,
        wasAutoApplied: true
      });
    ...
    // Record fix attempt for learning
    if (this.enableLearning && this.patterns) {
      this.patterns.recordFixAttempt(error, aiResult, applyResult.success, {
        duration: Date.now() - startTime,
        wasAutoApplied: true,
        sessionId: context.sessionId,
        userId: context.userId,
    ...
    // Record user feedback for personalization
    if (this.enablePersonalization && this.personalized && context.userId) {
      this.personalized.recordUserFeedback(context.userId, aiResult, applyResult.success, context);
      this.personalized.learnFromOutcome(context.userId, error, aiResult, applyResult.success, {
        ...context,
        duration: Date.now() - startTime,
        wasAutoApplied: true
      });
    ...
    // Record fix attempt for learning
    if (this.enableLearning && this.patterns) {
      this.patterns.recordFixAttempt(error, hybridResult, applyResult.success, {
        duration: Date.now() - startTime,
        wasAutoApplied: true,
        sessionId: context.sessionId,
        userId: context.userId,
    ...
    // Record user feedback for personalization
    if (this.enablePersonalization && this.personalized && context.userId) {
      this.personalized.recordUserFeedback(context.userId, hybridResult, applyResult.success, context);
      this.personalized.learnFromOutcome(context.userId, error, hybridResult, applyResult.success, {
        ...context,
        duration: Date.now() - startTime,
        wasAutoApplied: true
      });
      ...
      // Step 4: Generate personalized suggestions
      const baseSuggestions = this.generateSuggestions(error, templateResult, context);

      let suggestions = baseSuggestions;
      if (this.enablePersonalization && this.personalized) {
        // Get personalized recommendations
        const availableFixes = [templateResult, aiResult, hybridResult]
          .filter(result => result && result.success)
          .map(result => ({
            method: result.method,
            confidence: result.confidence,
            description: result.description || `${result.method} fix`,
            fix: result
          }));

        const personalizedRecs = this.personalized.getPersonalizedRecommendations(
          error,
          context,
          availableFixes
        );

        // Merge personalized recommendations with base suggestions
        suggestions = this.mergeSuggestions(baseSuggestions, personalizedRecs);
      }

      logger.info('Fix generation completed with suggestions', {
        suggestionsCount: suggestions.length,
        personalized: this.enablePersonalization
      });

      // Add prevention tips if available
      const preventionTips = this.enableLearning ? this.prevention.generatePreventionTips(context) : null;
      ...
      return {
        ...
        preventionTips: preventionTips ? {
          immediate: preventionTips.immediate.slice(0, 2),
          configuration: preventionTips.configuration.slice(0, 2),
          summary: `Prevention analysis: ${preventionTips.metrics.preventionPotential}% potential error reduction`
        } : null
      };
  ...
  /**
   * Merge base suggestions with personalized recommendations
   */
  mergeSuggestions(baseSuggestions, personalizedRecs) {
    const merged = new Map();

    // Add base suggestions
    baseSuggestions.forEach(suggestion => {
      const key = `${suggestion.type}_${suggestion.method || 'unknown'}`;
      merged.set(key, suggestion);
    });

    // Add or update with personalized recommendations
    personalizedRecs.forEach(personalized => {
      const key = `${personalized.type}_${personalized.method || 'unknown'}`;

      if (merged.has(key)) {
        // Update existing suggestion with personalized data
        const existing = merged.get(key);
        merged.set(key, {
          ...existing,
          ...personalized,
          confidence: personalized.personalizedScore || existing.confidence,
          personalized: true,
          personalizationFactors: personalized.personalizationFactors
        });
      } else {
        // Add new personalized suggestion
        merged.set(key, personalized);
      }
    });

    return Array.from(merged.values()).sort((a, b) => {
      // Sort by personalized score first, then confidence
      const aScore = a.personalizedScore || a.confidence || 0;
      const bScore = b.personalizedScore || b.confidence || 0;
