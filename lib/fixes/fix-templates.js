...
export class FixTemplates {
    ...
    this.templates = {
      ...
      syntax: {
        missing_semicolon: {
          ...
          fix: (error, content, context) => this.fixMissingSemicolon(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
        unexpected_token: {
          pattern: /unexpected token/i,
          fix: (error, content, context) => this.fixUnexpectedToken(error, content, context),
          confidence: 0.7,
          autoFixable: false, // Usually needs manual intervention
        },
        missing_bracket: {
          pattern: /missing.*bracket|unclosed.*bracket/i,
          fix: (error, content, context) => this.fixMissingBracket(error, content, context),
          confidence: 0.8,
          autoFixable: true,
        },
      },
      ...
      type: {
        cannot_find_name: {
          ...
          fix: (error, content, context) => this.fixCannotFindName(error, content, context),
          confidence: 0.6,
          autoFixable: false, // Usually needs import or declaration
        },
        property_not_exist: {
          pattern: /property.*does not exist/i,
          fix: (error, content, context) => this.fixPropertyNotExist(error, content, context),
          confidence: 0.5,
          autoFixable: false,
        },
        type_not_assignable: {
          pattern: /type.*is not assignable/i,
          fix: (error, content, context) => this.fixTypeNotAssignable(error, content, context),
          confidence: 0.4,
          autoFixable: false,
        },
      },
      ...
      import: {
        cannot_find_module: {
          ...
          fix: (error, content, context) => this.fixCannotFindModule(error, content, context),
          confidence: 0.8,
          autoFixable: true,
        },
        missing_import: {
          pattern: /missing import/i,
          fix: (error, content, context) => this.fixMissingImport(error, content, context),
          confidence: 0.7,
          autoFixable: true,
        },
      },
      ...
      unused: {
        unused_variable: {
          ...
          fix: (error, content, context) => this.fixUnusedVariable(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
        unused_import: {
          pattern: /unused import/i,
          fix: (error, content, context) => this.fixUnusedImport(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
        unreachable_code: {
          pattern: /unreachable code/i,
          fix: (error, content, context) => this.fixUnreachableCode(error, content, context),
          confidence: 0.8,
          autoFixable: true,
        },
      },
      ...
      scope: {
        not_defined: {
          ...
          fix: (error, content, context) => this.fixNotDefined(error, content, context),
          confidence: 0.5,
          autoFixable: false,
        },
        reference_error: {
          pattern: /reference error/i,
          fix: (error, content, context) => this.fixReferenceError(error, content, context),
          confidence: 0.4,
          autoFixable: false,
        },
      },
      ...
      style: {
        missing_quotes: {
          ...
          fix: (error, content, context) => this.fixQuotes(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
        indentation: {
          pattern: /indentation|indent/i,
          fix: (error, content, context) => this.fixIndentation(error, content, context),
          confidence: 0.8,
          autoFixable: true,
        },
        trailing_spaces: {
          pattern: /trailing|whitespace/i,
          fix: (error, content, context) => this.fixTrailingSpaces(error, content, context),
          confidence: 0.95,
          autoFixable: true,
        },
        no_console: {
          pattern: /unexpected console|console statement|Unexpected console/i,
          fix: (error, content, context) => this.fixConsoleStatement(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
        no_debugger: {
          pattern: /unexpected debugger/i,
          fix: (error, content, context) => this.fixDebuggerStatement(error, content, context),
          confidence: 0.9,
          autoFixable: true,
        },
      },
      ...
      react: {
        missing_key: {
          ...
          fix: (error, content, context) => this.fixMissingKey(error, content, context),
          confidence: 0.8,
          autoFixable: true,
        },
        jsx_syntax: {
          pattern: /jsx/i,
          fix: (error, content, context) => this.fixJsxSyntax(error, content, context),
          confidence: 0.6,
          autoFixable: false,
        },
      },
  ...
  fixMissingImport(error, fileContent) {
    ...
    return {
      success: false,
      reason: 'Missing import detection requires understanding of available modules',
    };
  }
  ...
  fixUnusedVariable(error, fileContent) {
    ...
    if (!varName) {
      return { success: false, reason: 'Could not extract variable name from error message' };
    }
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      const patterns = [
        ...
        new RegExp(`\\basync\\s+function\\s+${this.escapeRegExp(varName)}\\s*\\(`), // Async function
      ];
    ...
    for (let i = searchStart; i < searchEnd; i++) {
      ...
      const patterns = [
        ...
        new RegExp(`\\basync\\s+function\\s+${this.escapeRegExp(varName)}\\s*\\(`),
      ];
  ...
  fixUnusedImport(error, fileContent) {
    const importMatch = error.message.match(/['"](.+?)['"] is imported but never used/i);
    if (!importMatch) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes(`import ${importName}`) ||
  ...
  fixUnreachableCode(error, fileContent) {
    ...
    for (let i = startLine; i < lines.length; i++) {
      ...
      if (line.includes('return') || line.includes('throw') || line.includes('break')) {
        endLine = i + 1;
        break;
      }
    }
  ...
  fixTrailingSpaces(error, fileContent) {
    ...
    if (hasChanges) {
      return {
        ...
        changes: [{ type: 'cleanup', description: 'Removed trailing spaces from multiple lines' }],
      };
    }
  ...
  findImportsSection(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (start !== -1 && !line.startsWith('import ') && !line.startsWith('//') && line !== '') {
        end = i;
        break;
      }
    }
  ...
  getStatistics() {
    return {
      ...this.fixStats,
      successRate: this.fixStats.totalFixes > 0 ?
        (this.fixStats.successfulFixes / this.fixStats.totalFixes * 100).toFixed(1) : 0,
    };
  }
