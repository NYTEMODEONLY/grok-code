...
export class Logger {
  constructor(options = {}) {
    this.logDir = options.logDir || path.join(__dirname, '../../.grok');
    ...
    try {
      ...
      if (this.enableConsoleOutput) {
        console.warn('⚠️  Logger initialization failed, falling back to console-only logging:', error.message);
      }
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          if (this.enableConsoleOutput) {
            console.warn(`Log rotation check failed for ${file}:`, fileError.message);
          }
        }
      }
    } catch (error) {
