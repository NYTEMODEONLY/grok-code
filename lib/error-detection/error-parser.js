...
export class ErrorParser {
  ...
  detectSource(lines) {
    ...
    if (
      sampleLines.includes('TS') &&
      (sampleLines.includes('error') || sampleLines.includes('warning'))
    ) {
      return 'typescript';
    }

    if (
      sampleLines.includes('eslint') ||
      sampleLines.includes('no-unused-vars') ||
      sampleLines.includes('no-console')
    ) {
      return 'eslint';
    }

    if (
      sampleLines.includes('prettier') ||
      sampleLines.includes('[error]') ||
      sampleLines.includes('[warn]')
    ) {
      return 'prettier';
    }
  ...
  categorizeError(error) {
    ...
    if (code.startsWith('ts')) {
      ...
      if (['2304', '2307', '2322', '2339'].includes(tsCode))
        return 'type-error'; // Cannot find name, etc.
    ...
    // ESLint rules
    if (rule) {
      if (rule.includes('no-unused') || rule.includes('no-unreachable'))
        return 'unused-code';
      if (rule.includes('no-console') || rule.includes('no-debugger'))
        return 'debug-code';
      if (
        rule.includes('semi') ||
        ...
        return 'style-issue';
      if (rule.includes('no-undef') || rule.includes('no-var'))
        return 'scope-issue';
    ...
    if (message.includes('cannot find') || message.includes('module not found'))
      return 'import-error';
    if (message.includes('syntax') || message.includes('unexpected token'))
      return 'syntax-error';
    if (message.includes('type') || message.includes('interface'))
      return 'type-error';
    if (message.includes('unused') || message.includes('unreachable'))
      return 'unused-code';
  ...
  updateStats(error) {
    ...
    // By source
    this.parseStats.bySource[error.source] =
      (this.parseStats.bySource[error.source] || 0) + 1;

    // By severity
    this.parseStats.bySeverity[error.severity] =
      (this.parseStats.bySeverity[error.severity] || 0) + 1;

    // By file
    this.parseStats.byFile[error.file] =
      (this.parseStats.byFile[error.file] || 0) + 1;
  }
  ...
  exportErrors(format = 'json') {
    switch (format.toLowerCase()) {
      ...
      case 'summary':
        ...
        summary += 'By Severity:\n';
        Object.entries(this.parseStats.bySeverity).forEach(
          ([severity, count]) => {
