...
export class ContextErrorAnalyzer {
  ...
  async analyzeErrorsInContext(errorOutput, filePaths, options = {}) {
    ...
    const classifiedResult = errorClassifier.classifyErrors(
      parsedResult.errors
    );
    ...
    const impact = analyzeImpact
      ? this.analyzeSystemImpact(contextAwareErrors, context)
      : null;
    ...
    return {
      ...
      context: {
        ...
        dependenciesMapped: context.dependencies
          ? Object.keys(context.dependencies).length
          : 0,
        relevanceScores: context.relevance
          ? Object.keys(context.relevance).length
          : 0,
      },
      metadata: {
  ...
  async buildCodebaseContext(filePaths, options = {}) {
    ...
    try {
      ...
      if (includeDependencies) {
        const depResult = await dependencyMapper.buildDependencyGraph(
          filePaths,
          {
  ...
  async analyzeErrorContext(error, context, options = {}) {
    ...
    if (context.relevance && context.relevance[error.file]) {
      analysis.relevanceContext = {
        ...
        percentile: this.calculateRelevancePercentile(
          error.file,
          context.relevance
        ),
      };
    ...
    if (context.dependencies && context.dependents) {
      analysis.dependencyImpact = this.analyzeDependencyImpact(
        error.file,
        context
      );

      // Find files that might be affected by this error
      analysis.relatedFiles = this.findRelatedFiles(
        error,
        context,
        maxRelatedFiles
      );
    ...
    analysis.estimatedImpact = this.estimateErrorImpact(
      error,
      analysis,
      context
    );
    ...
    analysis.contextAdjustedComplexity = this.adjustComplexityForContext(
      error,
      analysis
    );
  ...
  calculateFileImportance(filePath, context) {
    ...
    importance.centrality =
      importance.dependencyCount + importance.dependentCount;
    ...
    if (context.files) {
      context.files.forEach((filePath) => {
        if (filePath !== error.file) {
          const similarity = this.calculateFileSimilarity(
            error,
            filePath,
            context
          );
  ...
  calculateFileSimilarity(error, filePath, context) {
    ...
    if (
      context.relevance &&
      context.relevance[error.file] &&
      context.relevance[filePath]
    ) {
      const relevanceDiff = Math.abs(
        context.relevance[error.file] - context.relevance[filePath]
      );
  ...
  estimateErrorImpact(error, analysis, context) {
    ...
    if (analysis.dependencyImpact.some((d) => d.risk === 'high'))
      impactScore += 1;
  ...
  identifyErrorPatterns(contextAwareErrors, context) {
    ...
    const highImpactErrors = contextAwareErrors.filter((error) =>
      ['systemic', 'critical'].includes(error.contextAnalysis.estimatedImpact)
    );
  ...
  analyzeSystemImpact(contextAwareErrors, context) {
    ...
    const highSeverityErrors = contextAwareErrors.filter(
      (e) => e.severityScore >= 4
    ).length;
  ...
  generateContextRecommendations(
    ...
    const prioritizedErrors = contextAwareErrors.sort((a, b) => {
      ...
      return (
        b.contextAnalysis.fileImportance.overall -
        a.contextAnalysis.fileImportance.overall
      );
    });
    ...
    if (impact && impact.riskAssessment === 'high') {
      recommendations.push({
        ...
        description:
          'High-risk error patterns detected - consider comprehensive testing before deployment',
      });
    }
  ...
  generateContextSummary(contextAwareErrors, patterns, impact) {
    ...
    // Add key insights
    const criticalErrors = contextAwareErrors.filter(
      (e) => e.contextAnalysis.estimatedImpact === 'critical'
    );
    ...
    const highImportanceFiles = contextAwareErrors.filter(
      (e) => e.contextAnalysis.fileImportance.overall > 60
    );
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            ...
            if (
              ['.js', '.ts', '.jsx', '.tsx', '.py', '.json', '.md'].includes(
                ext
              )
            ) {
              files.push(fullPath);
