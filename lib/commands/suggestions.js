...
export class ContextualSuggestions {
  ...
  generateSuggestions(context = {}) {
    try {
      ...
      for (const [analyzerName, analyzer] of Object.entries(
        this.contextAnalyzers
      )) {
        try {
          ...
          logger.warn(`Context analyzer ${analyzerName} failed`, {
            error: error.message,
          });
      ...
      const rankedSuggestions = this.rankAndFilterSuggestions(
  ...
  analyzeCommandContext(context) {
    ...
    recentCommands.slice(-10).forEach((cmd, index) => {
      ...
      analysis.recentCommandTypes.set(
        cmdType,
        (analysis.recentCommandTypes.get(cmdType) || 0) + 1
      );
      analysis.commandFrequency.set(
        cmd,
        (analysis.commandFrequency.get(cmd) || 0) + 1
      );
  ...
  analyzeConversationContext(context) {
    ...
    recentMessages.forEach((message) => {
      ...
      if (content.includes('?')) {
        ...
        if (content.includes('why'))
          analysis.questionTypes.set('explanation', true);
        if (content.includes('what'))
          analysis.questionTypes.set('information', true);
        if (content.includes('error') || content.includes('bug'))
          analysis.questionTypes.set('debugging', true);
      }

      // Detect topics
      if (content.includes('test')) analysis.recentTopics.add('testing');
      if (content.includes('error') || content.includes('bug'))
        analysis.recentTopics.add('debugging');
      if (content.includes('git') || content.includes('commit'))
        analysis.recentTopics.add('version-control');
      if (content.includes('file') || content.includes('directory'))
        analysis.recentTopics.add('file-management');
    });
    ...
    if (analysis.questionTypes.size > 2 || analysis.conversationLength > 10) {
      ...
      analysis.questionTypes.size > 0 ||
      analysis.conversationLength > 3
    ) {
  ...
  generateSuggestionsFromAnalysis(analyses) {
    ...
    if (analyses.conversation) {
      suggestions.push(
        ...this.generateConversationSuggestions(analyses.conversation)
      );
    }
  ...
  generateFileSuggestions(fileAnalysis) {
    ...
    if (
      fileAnalysis.recentFileTypes.has('.js') ||
      fileAnalysis.recentFileTypes.has('.ts')
    ) {
      suggestions.push({
        command: 'search',
        confidence: 0.5,
  ...
  generateCommandSuggestions(commandAnalysis) {
    ...
    const topCommand = this.getMostFrequentCommand(
      commandAnalysis.commandFrequency
    );
  ...
  applyPersonalization(suggestions, context) {
    ...
    return suggestions.map((suggestion) => {
      ...
      // Apply learning data
      const learnedPreference = this.learningData.userPreferences.get(
        suggestion.command
      );
  ...
  updateLearningData(context, suggestions) {
    try {
      ...
      suggestions.forEach((suggestion) => {
        const current =
          this.learningData.userPreferences.get(suggestion.command) || 1.0;
        // Slight preference boost for highly ranked suggestions
        const boost = suggestion.rank === 1 ? 1.05 : 1.02;
        this.learningData.userPreferences.set(
          suggestion.command,
          Math.min(current * boost, 2.0)
        );
      });
      ...
      if (context.recentCommands && context.recentCommands.length >= 2) {
        ...
        const current =
          this.learningData.commandSequences.get(sequenceKey) || 0;
