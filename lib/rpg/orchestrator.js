...
export class RPGOrchestrator {
  ...
  async executeFullRPGWorkflow(prompt, context = {}) {
    try {
      logger.info('Starting complete RPG workflow', { promptLength: prompt.length });
      ...
      const enhancedPlan = await this.enhancePlanWithIntelligence(basePlan, context);
      ...
      const implementation = await this.generateImplementation(validatedPlan, context);
      ...
      const summary = this.generateWorkflowSummary(validatedPlan, implementation);
      ...
      const finalResult = {
        ...
        metadata: {
          ...
          phases: ['planning', 'enhancement', 'validation', 'implementation', 'visualization']
        }
      };
...
Respond with valid JSON only.`;
    ...
    const response = await this.client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a precise RPG planner. Respond with valid JSON only. No explanations or markdown.'
        },
  ...
  async enhancePlanWithIntelligence(basePlan, context = {}) {
    ...
    try {
      ...
      const enhancedPlan = await this.smartRPG.enhanceRPGPlan(basePlan, context);
      ...
      logger.warn('Plan enhancement failed, using base plan', { error: error.message });
      return basePlan;
    }
  }
  ...
  async validateAndOptimizePlan(enhancedPlan) {
    ...
    if (validatedPlan.generatedCode) {
      for (const [filePath, code] of Object.entries(validatedPlan.generatedCode)) {
        if (code.placement && code.placement.path !== filePath) {
          ...
          validatedPlan.suggestions.push({
            ...
            reasoning: code.placement.reasoning || 'Better architectural placement'
          });
        }
      }
    }
  ...
  async generateImplementation(validatedPlan, context = {}) {
    ...
    try {
      ...
      if (validatedPlan.generatedCode) {
        ...
        implementation.files = await this.generateBasicImplementation(validatedPlan, context);
      }
  ...
  generateWorkflowSummary(plan, implementation) {
    ...
    // Recommendations
    summary.recommendations = this.generateRecommendations(plan, implementation);
  ...
  async saveRPGResult(rpgResult, fileName = null) {
    ...
    try {
      ...
      logger.error('Failed to save RPG result', { error: error.message, path: outputPath });
      throw error;
    }
  }
  ...
  async loadRPGPlan(filePath) {
    try {
      ...
      logger.error('Failed to load RPG plan', { error: error.message, path: filePath });
      throw error;
    }
  }
  ...
  validatePlanCompleteness(plan) {
    ...
    const unmappedFeatures = plan.features.filter(feature =>
      !Object.values(plan.files).includes(feature)
    );
  ...
  calculateCompleteness(plan, implementation) {
    ...
    return plannedFiles > 0 ? Math.round((implementedFiles / plannedFiles) * 100) : 0;
  }
  ...
  generateRecommendations(plan, implementation) {
    ...
    if (!plan.architecture?.pattern) {
      recommendations.push({
        type: 'architecture',
        priority: 'high',
        message: 'Consider specifying an architectural pattern (MVC, Layered, Clean, etc.)'
      });
    }
    ...
    if (!plan.frameworks || plan.frameworks.length === 0) {
      recommendations.push({
        type: 'frameworks',
        priority: 'medium',
        message: 'Specify technology frameworks for better implementation guidance'
      });
    }
  ...
  generateNextSteps(plan, implementation) {
    ...
    if (implementation.status === 'completed') {
      nextSteps.push({
        ...
        description: 'Examine the generated files and make any necessary adjustments'
      });

      nextSteps.push({
        step: 'Run tests',
        description: 'Execute the test suite to ensure everything works correctly'
      });
      ...
      nextSteps.push({
        step: 'Simplify plan',
        description: 'Consider breaking down complex plans into smaller, manageable pieces'
      });
    }
