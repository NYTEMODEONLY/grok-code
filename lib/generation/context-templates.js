...
export class ContextTemplateGenerator {
  ...
  async generateTemplate(templateType, name, context = {}) {
    try {
      ...
      this.applyArchitecturalPatterns(
        baseTemplate,
        projectContext,
        templateType
      );
      ...
      await this.applyProjectConventions(
        baseTemplate,
        projectContext,
        templateType
      );
      ...
      const placementSuggestion = await this.suggestFilePlacement(
        name,
        templateType,
        context
      );
  ...
  async analyzeProjectContext() {
    ...
    try {
      ...
      logger.warn('Project context analysis partially failed', {
        error: error.message,
      });
    }
    ...
    const templates = {
      ...
      model: {
        structure: {
          ...
          exports: [
            `export default mongoose.model('${this.pascalCase(name)}', ${this.pascalCase(name)}Schema);`,
          ],
        },
        patterns: {
      ...
      route: {
        structure: {
          imports: [
            `import ${this.camelCase(name)}Controller from '../controllers/${this.camelCase(name)}Controller';`,
          ],
      ...
      utility: {
        ...
        patterns: {
          errorHandling:
            'try { ... } catch (error) { console.error(error); return null; }',
        },
      },
  ...
  applyArchitecturalPatterns(template, projectContext, templateType) {
    ...
    if (architecture.pattern === 'Layered') {
      if (templateType === 'service') {
        template.structure.imports.push(
    ...
    try {
      ...
      return (
        suggestion.suggestions[0] || {
          path: `src/${templateType}s/${this.kebabCase(name)}.js`,
        }
      );
    } catch (error) {
    ...
    return {
      component: {
        ...
        transform: (template, name, context) => {
          ...
          template.structure.functionalComponent =
            template.structure.functionalComponent.replace(
              '{{name}}',
      ...
      service: {
        ...
        transform: (template, name, context) => {
          ...
          template.structure.functions = template.structure.functions.map(
            (fn) => fn.replace(/\{\{name\}\}/g, pascalName)
          );
        },
      },
    };
    ...
    return {
      component: {
        ...
        transform: (template, name, context) => {
          ...
          template.structure.decorator = template.structure.decorator.replace(
            /\{\{name\}\}/g,
            kebabName
          );
          template.structure.classDeclaration =
            template.structure.classDeclaration.replace(
              '{{name}}',
              this.pascalCase(name)
            );
        },
    ...
    return {
      ...
      view: {
        ...
        transform: (template, name, context) => {
          ...
          template.structure.functions = template.structure.functions.map(
            (fn) => fn.replace(/\{\{name\}\}/g, camelName)
          );
        },
      },
    };
    ...
    return {
      model: {
        structure: {
          ...
          fields: ['name: str', 'created_at: Optional[datetime] = None'],
        },
        transform: (template, name, context) => {
          template.structure.classDeclaration =
            template.structure.classDeclaration.replace(
      ...
      route: {
        ...
        transform: (template, name, context) => {
          ...
          template.structure.router = template.structure.router.replace(
            /\{\{name\}\}/g,
            camelName
          );
          template.structure.endpoints = template.structure.endpoints.map(
            (ep) => ep.replace(/\{\{name\}\}/g, camelName)
          );
        },
      },
    };
