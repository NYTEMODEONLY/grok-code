...
export class ErrorPatterns {
  ...
  getFrequentErrors(context = {}) {
    ...
    for (const [errorKey, patterns] of Object.entries(this.globalPatterns.error_frequency || {})) {
  ...
  getLocationHotspots(context = {}) {
    ...
    for (const [errorKey, patterns] of Object.entries(this.globalPatterns.error_locations || {})) {
  ...
  getFixEffectiveness(context = {}) {
    ...
    for (const [errorKey, fixRecords] of Object.entries(this.globalPatterns.fix_success_rates || {})) {
      const byMethod = {};
      ...
      Object.keys(byMethod).forEach(method => {
        ...
        stats.confidence = Math.min(stats.attempts / this.thresholds.patternMaturity, 1);
      });
  ...
  getPreventionSuggestions(context = {}) {
    ...
    if (typeErrors.length > 3) {
      suggestions.push({
        type: 'type_checking',
        priority: 'high',
        description: 'Enable strict TypeScript checking or add type annotations',
  ...
  generateRecommendations(context = {}) {
    ...
    for (const [errorKey, methods] of Object.entries(fixEffectiveness)) {
      const bestMethod = Object.entries(methods)
        .filter(([_, stats]) => stats.confidence >= this.thresholds.confidenceThreshold)
  ...
  generateInsights(context = {}) {
    ...
    const worstFileType = Object.entries(fileTypeErrors)
      .sort(([,a], [,b]) => b - a)[0];
    ...
    const totalPatterns = Object.values(this.globalPatterns)
  ...
  savePatterns() {
    try {
      ...
      fs.writeFileSync(this.patternsFile, JSON.stringify(this.globalPatterns, null, 2));
  ...
  pruneOldPatterns() {
    ...
    for (const category of Object.values(this.categories)) {
      if (this.globalPatterns[category] && typeof this.globalPatterns[category] === 'object') {
        for (const [key, patterns] of Object.entries(this.globalPatterns[category])) {
  ...
  getTotalPatternCount() {
    return Object.values(this.globalPatterns)
      .reduce((sum, category) => {
        if (Array.isArray(category)) {
          return sum + category.length;
        } else if (typeof category === 'object') {
          return sum + Object.values(category)
            .reduce((catSum, patterns) => catSum + (Array.isArray(patterns) ? patterns.length : 0), 0);
        }
        return sum;
      }, 0);
  }
  ...
  getStats() {
    return {
      ...
      frequentErrors: Object.keys(this.globalPatterns[this.categories.frequency] || {}).length,
      locationHotspots: Object.keys(this.globalPatterns[this.categories.location] || {}).length,
      fixRecords: Object.keys(this.globalPatterns[this.categories.fix_success] || {}).length,
