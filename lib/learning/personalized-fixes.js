...
export class PersonalizedFixes {
  ...
  getPersonalizedRecommendations(error, context, availableFixes = []) {
    ...
    const patternAnalysis = this.patterns.analyzePatterns({
      userId,
      projectId,
    });
  ...
  recordUserFeedback(userId, fix, accepted, context = {}) {
    ...
    if (accepted) {
      ...
      preference[method] =
        (preference[method] || 0.5) - this.learningRate * 0.5;
      preference[method] = Math.max(0.0, preference[method]); // Floor at 0.0

      // Track rejected methods
      const rejected =
        this.userPreferences.rejectedMethods.get(userId) || new Set();
      rejected.add(method);
  ...
  learnFromOutcome(userId, error, fix, success, context = {}) {
    ...
    effectiveness[method].successRate =
      effectiveness[method].successes / effectiveness[method].attempts;
  ...
  personalizeRecommendations(
    ...
    const personalized = availableFixes.map((fix) => {
      ...
      if (userProfile.methodEffectiveness?.[errorKey]?.[method]) {
        const successRate =
          userProfile.methodEffectiveness[errorKey][method].successRate;
        score += (successRate - 0.5) * 0.4; // Boost/cut based on personal success rate
      }

      // Consider project patterns
      const projectPatterns =
        this.userPreferences.projectPatterns.get(userId)?.[projectId];
  ...
  updateUserProfile(userId, context) {
    ...
    if (
      context.projectId &&
      !profile.projectHistory.includes(context.projectId)
    ) {
  ...
  boostSimilarMethods(preferences, preferredMethod) {
    ...
    similars.forEach((method) => {
      if (method !== preferredMethod) {
        preferences[method] =
          (preferences[method] || 0.5) + this.learningRate * 0.3;
  ...
  updateProjectPatterns(userId, projectId, fix, success) {
    ...
    const method = fix.method || 'unknown';
    project.preferredMethods[method] =
      (project.preferredMethods[method] || 0.5) +
      (success ? this.learningRate : -this.learningRate * 0.5);
    project.preferredMethods[method] = Math.max(
      0,
      Math.min(1, project.preferredMethods[method])
    );
  ...
  getUserInsights(userId) {
    ...
    const projectStats = Object.entries(projects).map(([projectId, stats]) => ({
      projectId,
      successRate:
        stats.totalFixes > 0 ? stats.successfulFixes / stats.totalFixes : 0,
      totalFixes: stats.totalFixes,
      topMethod: Object.entries(stats.preferredMethods || {}).sort(
        ([, a], [, b]) => b - a
      )[0]?.[0],
    }));
    ...
    return {
      ...
      preferences: {
        topMethods,
        rejectedMethods: Array.from(
          this.userPreferences.rejectedMethods.get(userId) || []
        ),
      },
  ...
  getPersonalizationStats() {
    ...
    const totalProjects = Array.from(
      this.userPreferences.projectPatterns.values()
  ...
  exportPersonalizationData() {
    return {
      ...
      userPreferences: {
        preferredMethods: Object.fromEntries(
          this.userPreferences.preferredMethods
        ),
        rejectedMethods: Object.fromEntries(
          Array.from(this.userPreferences.rejectedMethods.entries()).map(
            ([k, v]) => [k, Array.from(v)]
          )
        ),
        projectPatterns: Object.fromEntries(
          this.userPreferences.projectPatterns
        ),
        timePreferences: Object.fromEntries(
          this.userPreferences.timePreferences
        ),
      },
      metadata: {
  ...
  importPersonalizationData(data) {
    ...
    if (data.userPreferences) {
      ...
      this.userPreferences.rejectedMethods = new Map(
        Object.entries(data.userPreferences.rejectedMethods || {}).map(
          ([k, v]) => [k, new Set(v)]
        )
      );
