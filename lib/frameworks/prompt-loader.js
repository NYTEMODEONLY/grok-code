...
export class FrameworkPromptLoader {
  constructor(options = {}) {
    ...
    logger.debug('Framework prompt loader initialized', {
...
- Module organization and exports`,
  ...
  loadPrompts(detectedFrameworks) {
    ...
    if (frameworksWithPrompts.length === 0) {
      prompts.systemPrompt =
        'You are working with a general software development project. Provide helpful, accurate coding assistance.';
      return prompts;
    }
  ...
  getContextualPrompt(detectedFrameworks, operationType = 'general') {
    const frameworks = detectedFrameworks.filter(
      (fw) => this.frameworkPrompts[fw.name.toLowerCase()]
    );
