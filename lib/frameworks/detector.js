...
export class FrameworkDetector {
  constructor(options = {}) {
    ...
    this.frameworks = {
      ...
      react: {
        ...
        indicators: {
          ...
          files: ['src/App.js', 'src/App.tsx', 'src/index.js', 'public/index.html'],
          extensions: ['.jsx', '.tsx'],
          patterns: ['ReactDOM\\.render', 'import React from \'react\'', 'function.*Component']
        },
        confidence: 0
      },
      vue: {
        ...
        indicators: {
          ...
          extensions: ['.vue'],
          patterns: ['new Vue', 'import Vue from \'vue\'', '<template>', '<script>', '<style>']
        },
        confidence: 0
      },
      angular: {
      ...
      express: {
        ...
        indicators: {
          ...
          patterns: ['app\\.listen', 'express\\(\\)', 'app\\.get', 'app\\.post']
        },
        confidence: 0
      },
      fastify: {
        ...
        indicators: {
          ...
          patterns: ['fastify\\.register', 'fastify\\.listen', 'fastify\\.get', 'fastify\\.post']
        },
        confidence: 0
      },
      nestjs: {
      ...
      django: {
        ...
        indicators: {
          ...
          patterns: ['django', 'DJANGO_SETTINGS_MODULE', 'urlpatterns', 'admin\\.site']
        },
        confidence: 0
      },
      flask: {
      ...
      fastapi: {
        ...
        indicators: {
          ...
          patterns: ['FastAPI', '@app\\.get', '@app\\.post', 'Depends', 'HTTPException']
        },
        confidence: 0
      },
      ...
      nextjs: {
        ...
        indicators: {
          ...
          patterns: ['getServerSideProps', 'getStaticProps', 'Link from \'next/link\'']
        },
        confidence: 0
      },
      nuxtjs: {
      ...
      vite: {
        ...
        category: 'build-tool',
        indicators: {
          devDependencies: ['vite', '@vitejs/plugin-react', '@vitejs/plugin-vue'],
      ...
      tailwind: {
        ...
        indicators: {
          ...
          patterns: ['className="[^"]*bg-', 'className="[^"]*text-', 'className="[^"]*flex']
        },
        confidence: 0
      },
      bootstrap: {
        ...
        indicators: {
          ...
          patterns: ['className="[^"]*container', 'className="[^"]*row', 'className="[^"]*col']
        },
        confidence: 0
      }
    };
    ...
    this.languages = {
      ...
      typescript: {
        name: 'TypeScript',
        extensions: ['.ts', '.tsx'],
        patterns: ['interface ', 'type ', ': string', ': number', ': boolean', '<T>']
      },
      python: {
        name: 'Python',
        extensions: ['.py'],
  ...
  async detectFrameworks() {
    try {
      if (logger && logger.info) logger.info('Starting framework detection', { projectRoot: this.projectRoot });
      ...
      if (logger && logger.error) logger.error('Framework detection failed', { error: error.message });
      return {
        frameworks: [],
        languages: [],
        error: error.message
  ...
  async loadPackageJson() {
    try {
      ...
      if (logger && logger.debug) logger.debug('Package.json loaded', {
      ...
      if (logger && logger.debug) logger.debug('No package.json found or failed to parse', { error: error.message });
    }
  }
    ...
    try {
      ...
      for (const item of items) {
        if (item.isDirectory()) {
          ...
          if (['node_modules', '.next', '.nuxt', 'dist', 'build', 'public', 'src'].includes(item.name)) {
          ...
          if (['package.json', 'requirements.txt', 'Pipfile', 'Gemfile', 'composer.json', 'Cargo.toml'].includes(item.name)) {
            structure.keyFiles.add(item.name);
          }
        }
      }

      if (logger && logger.debug) logger.debug('Project structure analyzed', structure);
      return structure;

    } catch (error) {
      if (logger && logger.error) logger.error('Failed to analyze project structure', { error: error.message });
      return structure;
    }
  }
  ...
  async detectLanguages() {
    ...
    for (const [langKey, langInfo] of Object.entries(this.languages)) {
      ...
      if (languageScore > 0) {
        ...
        for (const filePath of sampleFiles.slice(0, 3)) { // Check up to 3 files
          try {
            ...
            if (patternMatches.length > 0) {
              languageScore += 0.4 * (patternMatches.length / langInfo.patterns.length);
            }
          } catch (error) {
      ...
      if (languageScore >= 0.5) {
        ...
        if (logger && logger.debug) logger.debug(`Language detected: ${langInfo.name}`, { score: languageScore });
      }
    }
  }
  ...
  async findSampleFiles(extensions) {
    ...
    try {
      ...
      if (logger && logger.debug) logger.debug('Error finding sample files', { error: error.message });
    }
  ...
  async analyzeFramework(frameworkKey) {
    ...
    if (this.packageJson) {
      ...
      if (indicators.devDependencies) {
        ...
        score += indicators.devDependencies.filter(dep => allDeps[dep]).length;
      }
    }
    ...
    if (indicators.extensions) {
      ...
      const projectExtensions = Object.keys(this.projectStructure?.fileTypes || {});
      score += indicators.extensions.filter(ext => projectExtensions.includes(ext)).length;
    }
    ...
    if (logger && logger.debug) logger.debug(`Framework analysis: ${framework.name}`, {
      score,
      maxScore,
  ...
  async patternExists(pattern, extensions) {
    try {
      const sampleFiles = extensions ? await this.findSampleFiles(extensions) : await this.findSampleFiles(['.js', '.ts', '.py', '.java']);

      for (const filePath of sampleFiles.slice(0, 5)) { // Check up to 5 files
        try {
          const content = await fs.readFile(filePath, 'utf8');
      ...
      if (logger && logger.debug) logger.debug('Error checking pattern', { pattern, error: error.message });
    }
  ...
  calculateConfidenceScores() {
    for (const [key, framework] of Object.entries(this.frameworks)) {
      // Boost confidence for frameworks with strong indicators
  ...
  getOverallConfidence() {
    ...
    const avgConfidence = detected.reduce((sum, fw) => sum + fw.confidence, 0) / detected.length;
  ...
  getRecommendations() {
    ...
    if (detectedLangs.includes('javascript') || detectedLangs.includes('typescript')) {
      recommendations.push(
        { framework: 'react', reason: 'Popular UI library for web development' },
        { framework: 'express', reason: 'Minimalist web framework for Node.js' },
    ...
    if (detectedLangs.includes('python')) {
      recommendations.push(
        ...
        { framework: 'fastapi', reason: 'Modern API framework with async support' }
      );
    }
