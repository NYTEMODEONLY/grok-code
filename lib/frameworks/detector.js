...
export class FrameworkDetector {
    ...
    this.frameworks = {
      ...
      nodejs: {
        ...
        indicators: {
          ...
          patterns: ['require\\(', 'import.*from', 'module\\.exports', 'exports\\.']
        },
        confidence: 0
      }
    };
  ...
  async detectFrameworks() {
    try {
      logger.info('Starting framework detection', { projectRoot: this.projectRoot });
  ...
  async loadPackageJson() {
    try {
      ...
      logger.debug('No package.json found or failed to parse', { error: error.message });
    }
  }
  ...
  async analyzeProjectStructure() {
    ...
    try {
      ...
      logger.error('Failed to analyze project structure', { error: error.message });
      return structure;
    }
  }
  ...
  async detectLanguages() {
    ...
    for (const [langKey, langInfo] of Object.entries(this.languages)) {
      ...
      if (languageScore >= 0.5) {
        ...
        logger.debug(`Language detected: ${langInfo.name}`, { score: languageScore });
      }
    }
  }
