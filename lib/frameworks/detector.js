...
export class FrameworkDetector {
  ...
  async loadPackageJson() {
    try {
      ...
      let packagePath = path.join(this.projectRoot, 'package.json');
      let packageContent;

      if (await fs.pathExists(packagePath)) {
        packageContent = await fs.readFile(packagePath, 'utf8');
      } else {
        // Look for package.json in the project structure
        const keyFiles = Array.from(this.projectStructure?.keyFiles || []);
        const packageJsonPath = keyFiles.find(file => file.endsWith('package.json'));

        if (packageJsonPath) {
          packagePath = path.join(this.projectRoot, packageJsonPath);
          packageContent = await fs.readFile(packagePath, 'utf8');
        }
      }
    ...
    const scanDirectory = async (dirPath, relativePath = '') => {
      try {
        ...
        for (const item of items) {
          ...
          if (item.isDirectory()) {
            // Skip certain directories to avoid scanning huge trees
            if (['node_modules', '.git', '.next', '.nuxt', 'dist', 'build'].includes(item.name)) {
              continue;
            }

            structure.totalDirectories++;
            structure.directories.add(relPath);
            ...
            if (
              ...
              structure.keyFiles.add(relPath + '/');
            }
        ...
        logger.debug('Skipping directory', { path: dirPath, error: error.message });
      }
    };
