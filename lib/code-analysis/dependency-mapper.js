...
export class DependencyMapper {
  ...
  async buildDependencyGraph(inputPaths, options = {}) {
    ...
    for (const filePath of files) {
      try {
        const result = await this.analyzeFileDependencies(filePath, {
          cacheEnabled,
        });
        results.push(result);
      } catch (error) {
  ...
  extractDependencies(astData, symbols, filePath) {
    ...
    if (astData.language === 'python') {
      const packageDeps = this.extractPythonPackageDependencies(
        filePath,
        symbols
      );
    ...
    for (const inputPath of inputPaths) {
      if (fs.existsSync(inputPath)) {
        ...
        if (stats.isDirectory()) {
          ...
          const dirFiles = await this.collectFromDirectory(
            inputPath,
            includePatterns,
            excludePatterns
          );
          ...
          // Single file
          if (
            this.matchesPatterns(inputPath, includePatterns, excludePatterns)
          ) {
      ...
      for (const entry of entries) {
        ...
        if (stats.isDirectory()) {
          ...
          if (!this.matchesExcludePatterns(fullPath, excludePatterns)) {
            const subFiles = await this.collectFromDirectory(
              fullPath,
              includePatterns,
              excludePatterns
            );
          ...
          if (
            this.matchesPatterns(fullPath, includePatterns, excludePatterns)
          ) {
            files.push(fullPath);
          }
        }
