...
export class CircularDependencyDetector {
  ...
  generateRecommendations(analysis) {
    ...
    if (analysis.totalCycles === 0) {
      return [
        '✅ No circular dependencies detected. Good job maintaining clean architecture!',
      ];
    }

    // General recommendations
    recommendations.push(
      '🔄 Circular dependencies detected. Consider these fixes:'
    );
    recommendations.push('');
    ...
    recommendations.push(
      '  • Use abstract interfaces instead of concrete classes'
    );
    recommendations.push(
      '  • Consider module boundaries and single responsibility'
    );
    recommendations.push(
      '  • Use tools like madge or dependency-cruiser for ongoing monitoring'
    );
