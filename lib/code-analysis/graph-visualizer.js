...
export class GraphVisualizer {
  ...
  visualize(dependencyGraph, reverseGraph = new Map(), options = {}) {
    ...
    switch (style) {
      case 'tree':
        visualization += this.generateTreeView(dependencyGraph, rootNode, maxDepth, showCycles);
        break;
  ...
  generateStats(dependencyGraph, reverseGraph) {
    ...
    const revMap = reverseGraph instanceof Map ? reverseGraph : new Map(Object.entries(reverseGraph || {}));
    ...
    stats.avgDependencies = depMap.size > 0 ? (totalDeps / depMap.size).toFixed(1) : 0;
  ...
  generateTreeView(dependencyGraph, rootNode, maxDepth, showCycles) {
    ...
    if (rootNode && depMap.has(rootNode)) {
      ...
      tree = this.buildTreeFromNode(rootNode, depMap, '', maxDepth, showCycles, new Set());
    } else {
      ...
      for (let i = 0; i < Math.min(rootNodes.length, 5); i++) {
        ...
        tree += this.buildTreeFromNode(root, depMap, '', maxDepth, showCycles, new Set());
   ...
   */
  buildTreeFromNode(node, dependencyGraph, prefix, maxDepth, showCycles, visited) {
    ...
    for (let i = 0; i < deps.length; i++) {
      ...
      const branchSymbol = isLast ? this.symbols.lastBranch : this.symbols.branch;
      const nextPrefix = prefix + (isLast ? this.symbols.space : this.symbols.vertical);
      ...
      if (maxDepth > 1 && dependencyGraph.has(dep.path)) {
        tree += this.buildTreeFromNode(dep.path, dependencyGraph, nextPrefix, maxDepth - 1, showCycles, new Set(visited));
      }
    }
...
`;
  ...
  export(visualization, format = 'text') {
    switch (format.toLowerCase()) {
      ...
      case 'json':
        return JSON.stringify({ visualization, timestamp: new Date().toISOString() }, null, 2);
  ...
  validateOptions(options) {
    ...
    if (!this.getAvailableStyles().includes(validated.style)) {
      throw new Error(`Invalid style: ${validated.style}. Available: ${this.getAvailableStyles().join(', ')}`);
    }
